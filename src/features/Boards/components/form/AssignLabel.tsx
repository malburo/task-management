...
const AssignLabel: React.FC<Props> = ({ task }) => {
  ...
  const labels: ILabel[] = useSelector(labelsSelector.selectAll);
  ...
  return (
    <Box>
      ...
      >
        <Box padding="16px" minHeight="100px" width="220px">
          ...
          {mode === 'ASSIGN' && (
            <>
              ...
              <Box padding="12px" marginY="24px" boxShadow="0px 4px 12px rgba(0, 0, 0, 0.1)" borderRadius="12px">
                {labels.map((label) => (
                  <Box display="flex" alignItems="center" justifyContent="space-between" key={label._id}>
                    ...
                    >
                      <Typography sx={{ color: 'white' }} variant="regular2">
                        {label.name}
                      </Typography>
                      {task.labelsId.includes(label._id) && <CheckIcon sx={{ color: 'white', fontSize: '16px' }} />}
                    </Box>
                    <IconButton
